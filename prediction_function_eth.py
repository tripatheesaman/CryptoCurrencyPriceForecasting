...
def model_selection(a,pred_days):
    if a == "ETH_lstm":
        model = tf.keras.models.load_model('lstmmodel_ETH')
    elif a == "ETH_gru":
        model = tf.keras.models.load_model('grumodel_ETH')
    else:
        model = null
        
    error_calculation(model)
    lst_output = prediction_function(model,pred_days)
    plot_function(model,lst_output, pred_days)
    return
